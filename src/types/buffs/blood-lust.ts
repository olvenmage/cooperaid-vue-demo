...
export default class BloodLustBuff extends Buff {
    ...
    increaseNextAttack(trigger: DealDamageToParams, damagedBy: Character): DealDamageToParams {
        if (damagedBy.id == this.givenBy?.id && trigger.type == DamageType.PHYSICAL) {
            trigger.amount += this.params.healthConsumed
            this.endEffect(this.givenBy)
        }
